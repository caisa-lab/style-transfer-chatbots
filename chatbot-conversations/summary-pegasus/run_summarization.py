...
def main():
    ...
    # Init wandb
    projectName = 'summary-pegasus-debate-sum'
    ...
    # set warmup steps to 6% of total steps
    if training_args.do_train:
        totalSteps = len(datasets['train']) * training_args.num_train_epochs
        tsDivisor = training_args.per_device_train_batch_size
        if (training_args.eval_accumulation_steps):
            tsDivisor *= training_args.eval_accumulation_steps
        totalSteps = totalSteps // tsDivisor
        warmupSteps = int(0.06 * totalSteps)
        print('Total steps:', totalSteps)
        print('Warmup steps:', warmupSteps)
        training_args.warmup_steps = warmupSteps
