#!/bin/bash
#SBATCH --job-name=finetune_roberta_%dataset%
#SBATCH --output="/ukp-storage-1/nothvogel/%dataset%-classifier/logs/log_%dataset%.txt"
...
# Experiment Details :- RoBERTa model for %dataset%.
# Run Details :- none
...
DATA_DIR="/ukp-storage-1/nothvogel/style-dataset-preprocessing/data/%dataset%"

# replace %dataset% with name of dataset!
# also replace labels and max epochs!

MAX_EPOCHS=10

SCRIPT_PATH="/ukp-storage-1/nothvogel/%dataset%-classifier/train_classifier.py"
